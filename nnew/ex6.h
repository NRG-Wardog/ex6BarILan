...
#define FAIL -1
#define SIZE 152
#define ATTACK_RATIO (double)1.5
#define HP_RATIO (double)1.2
#define DOUBLE 2
...
typedef enum StarterPokemon {
     BULBASAUR = 0,
     CHARMANDER = 3,
     SQUIRTLE = 6,
}StarterPokemon;

typedef enum StarterPokemonCase {
     BULBASAUR_CASE = 1,
     CHARMANDER_CASE,
     SQUIRTLE_CASE,
}StarterPokemonCase;
...
/**
 * @brief Recursively merges all Pokemon from a second pokedex tree into the first owner's pokedex.
 * @param first Pointer to the OwnerNode to merge into.
 * @param pokdexSecond Root of the second owner's pokedex tree.
 * @return TRUE (1) on success, FAIL (-1) on allocation failure.
 * The function allocates new PokemonNode copies as needed.
 */
int mergeTwoPokadex(OwnerNode* first, PokemonNode *pokdexSecond );

/**
 * @brief Calculates the battle power score for a Pokemon based on its attack and HP.
 * @param attack The attack value of the Pokemon.
 * @param hp The HP (health points) value of the Pokemon.
 * @return The calculated power score as a double.
 */
double powerPokemonFight(const int attack,const int hp);

/**
 * @brief Copies all data fields from one PokemonData struct to another, allocating and copying the name.
 *        If dst->name is already allocated, it will be freed before copying.
 * @param dst Pointer to the destination PokemonData struct.
 * @param src Pointer to the source PokemonData struct.
 * @return TRUE (1) on success, FAIL (-1) on memory allocation failure.
 * @note Caller is responsible for ensuring dst is properly allocated.
 */
int copyPokemonData(PokemonData *dst, const PokemonData *src);

/**
 * @brief Finds and returns the node with the minimum ID in the given Pokemon BST.
 * @param root Pointer to the BST root.
 * @return Pointer to the node with the minimum ID, or NULL if the tree is empty.
 */
PokemonNode *findMinNode(PokemonNode *root);

/**
 * @brief Sorts an array of PokemonNode pointers by name (alphabetically) using bubble sort.
 * @param pokdex Pointer to the NodeArray to sort.
 */
void bubbleSortByNames(const NodeArray* pokdex);

/**
 * @brief Prints all PokemonNodes stored in a NodeArray, in order.
 * @param pokdex Pointer to the NodeArray to print.
 */
void printArray(const NodeArray* pokdex);

/**
 * @brief Frees all nodes in a queue of QueuePokemon nodes.
 * @param head Pointer to the head of the queue (may be NULL).
 */
void freeEntireQueue(QueuePokemon *head);

/**
 * @brief Adds a PokemonNode pointer to the end of a QueuePokemon singly-linked queue.
 * @param q Pointer to the pointer to the queue head.
 * @param pokemonNode Pointer to the PokemonNode to add (must not be NULL).
 * @return TRUE (1) on success, FAIL (-1) on allocation failure.
 */
int add(QueuePokemon **q, PokemonNode *pokemonNode);

/**
 * @brief Prints all owners in the circular linked list, once, in order.
 * Does not prompt the user.
 */
void printOwnersList();

/**
 * @brief Returns a pointer to the OwnerNode at the given index (1-based) in the circular list.
 * @param id Index of the owner to return (starting from 1).
 * @return Pointer to the OwnerNode at the given position, or NULL if index is invalid.
 */
OwnerNode* getOwnerByIndex(const int id);

/**
 * @brief Frees memory allocated for a PokemonData struct, including its name.
 * @param cur Pointer to the PokemonData struct to free (may be NULL).
 * Only frees the name field; does not free the struct itself.
 */
